...
const utils = {
  ...
  oneOf<T>(key: string, expectedValues?: unknown[], defaultValue?: T) {
    if (!expectedValues) {
    ...
    const rawValue = env<T>(key, defaultValue);
    return (expectedValues.includes(rawValue) ? rawValue : defaultValue) as T;
  },

  getOrThrow<T = string>(key: string) {
    if (!_.has(process.env, key)) {
      throw new Error(`process.env key "${key}" does not exist`);
    }
    return getKey(key) as T;
  },
};
